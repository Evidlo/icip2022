...
def minimize(f, X, fprime, length, args=(), reduction=None, verbose=True, concise=False):
	"""
	...
	f0 = eval('f')(X, args)			# get initial function value and gradient
	df0 = eval('fprime')(X, args)			# get initial function value and gradient
	df0 = df0.reshape(-1, 1)
	fX=[]; fX.append(f0)
	Xd=[]; Xd.append(X)
